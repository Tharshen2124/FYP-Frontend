...
export function WeeklySummaryCard({ summary, hasAnyReflection, generating, onGenerate }: Props) {
  return (
    <div className="p-6 rounded-2xl bg-card border-2 border-border mb-8">
      <div className="flex items-center justify-between mb-3">
        ...
        {!summary ? (
          <Button
            onClick={onGenerate}
            disabled={generating || !hasAnyReflection}
          ...
          >
            {generating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4 mr-2" />}
            Regenerate
          </Button>
        )}
      </div>

...
export function DeleteRoleModal({ role, onCancel, onConfirm }: Props) {
  return (
    <AlertDialog open={!!role} onOpenChange={open => { if (!open) onCancel() }}>
      <AlertDialogContent className="bg-card border-border text-foreground">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-muted-foreground font-serif">
            ...
            <span className="font-bold text-foreground">{role?.goals.length} {role?.goals.length === 1 ? "goal" : "goals"}</span> associated with it.
